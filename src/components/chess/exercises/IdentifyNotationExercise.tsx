...
export function IdentifyNotationExerciseComponent({ exercise, onCorrect }: Props) {
  ...
  function getOptionClass(option: string) {
    ...
    if (!selected) {
      return `${base} border-[#2a2a2a] bg-[#1a1a1a] text-[#a0a0a0] hover:border-[#c8a84b] hover:text-[#f0f0f0] cursor-pointer`
    }
    if (option === exercise.square) {
      return `${base} border-[#4ade80] bg-[rgba(74,222,128,0.15)] text-[#4ade80] cursor-default`
    }
    if (option === selected && option !== exercise.square) {
      return `${base} border-[#f87171] bg-[rgba(248,113,113,0.15)] text-[#f87171] cursor-default`
    }
    return `${base} border-[#2a2a2a] bg-[#1a1a1a] text-[#555] cursor-default`
  }

  const highlight = exercise.fen
    ? { [exercise.square]: { background: 'rgba(200,168,75,0.5)' } }
    : undefined
